...
class GNN(torch.nn.Module):
    def __init__(self, num_hidden_chnls, gnn_op_type):
        ...
        if gnn_op_type == cfg.GNN_OP_ID_RESGATEDGRAPHCONV:
            ...
            self.graphOperator1 = TransformerConv(num_hidden_chnls, num_hidden_chnls, heads=cfg.GTR_NUM_HEADS, concat=False, edge_dim=cfg.NUM_FEAT_INTERACTION)
            self.graphOperator2 = TransformerConv(num_hidden_chnls, num_hidden_chnls, heads=cfg.GTR_NUM_HEADS, concat=False, edge_dim=cfg.NUM_FEAT_INTERACTION)
        elif gnn_op_type == cfg.GNN_OP_ID_GINE:
            self.graphOperator1 = GINEConv(torch.nn.Sequential(torch.nn.Linear(num_hidden_chnls, num_hidden_chnls)), eps=0.0, train_eps=cfg.GINE_TRAIN_EPS, edge_dim=cfg.NUM_FEAT_INTERACTION)
            self.graphOperator2 = GINEConv(torch.nn.Sequential(torch.nn.Linear(num_hidden_chnls, num_hidden_chnls)), eps=0.0, train_eps=cfg.GINE_TRAIN_EPS, edge_dim=cfg.NUM_FEAT_INTERACTION)
        else:
